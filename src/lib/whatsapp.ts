...
export function generateWhatsAppMessage(items: CartItem[], subtotal: number, customer: CustomerDetails): string {
  ...
  const orderSection = "*Order Details:*\n" + items.map((item, index) => {
    let weightLabel = '';
    if (item.selectedWeight === 'quarter_kg') weightLabel = ' (¼kg)';
  ...
  return header + customerSection + orderSection + footer;
}
